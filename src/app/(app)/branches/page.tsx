...
const FALLBACK_BRANCHES: ErpBranch[] = [
  ...
  { id: 2, iasCompanyId: 2, name: "Mombasa Branch", code: "MSA", status: "ACTIVE" },
];
...
export default function BranchesPage() {
  ...
  const columns: Column<ErpBranch>[] = [
    ...
    { header: "Code", accessor: (b) => <span className="font-mono text-xs">{b.code}</span> },
    {
      header: "Status",
      accessor: (b) => <Badge tone={b.status === "ACTIVE" ? "green" : "neutral"}>{b.status}</Badge>,
    },
  ];

  return (
    <>
      <Topbar title="Branches" description="Physical locations within this company." />
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4">
        <div className="flex items-center justify-between gap-3">
          <p className="text-xs text-ink-500">Branches can be created here, but aren&apos;t editable after creation.</p>
          <Link
            href="/branches/new"
        ...
        <DataTable columns={columns} rows={branches} rowKey={(b) => String(b.id)} />
      </div>
    </>
  );
}
